function S_STRUCT = MSUNIWARD_SP(coverPath, payload)

% -------------------------------------------------------------------------
% chenkejiang 2016.5.13
% -------------------------------------------------------------------------
...
for bcoord_i=1:8
    for bcoord_j=1:8
       ...
       % testCoeffs1 = ones(8, 8);
        %testCoeffs1(bcoord_i, bcoord_j) = 0;
        %considerate +1 and -1 impact  by  chenkejiang
        spatialImpact{bcoord_i, bcoord_j} = idct2(testCoeffs)*C_QUANT(bcoord_i, bcoord_j); %;+ idct2(testCoeffs1)*C_QUANT(bcoord_i, bcoord_j))/2;
    end
end
...
RC_UM = cell(size(F));
for i=1:numel(F)
    RC_UM{i} = imfilter(C_SPATIAL_PADDED_UM, F{i});
end
...
for row = 1:k
    for col = 1:l
        ...
        for fIndex = 1:3
            ...
            RC_UM_sub = RC_UM{fIndex}(subRows, subCols); 
